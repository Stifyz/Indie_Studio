...
public:
    ...
    void injectKeyDown(const KeyboardEvent& evt)
    {
        ...
        if (key == 'a' && (mTopAnimID == ANIM_IDLE_TOP || mTopAnimID == ANIM_RUN_TOP))
        {
        ...
        else if (key == 'z') mKeyDirection.z = -1;
        else if (key == 'q') mKeyDirection.x = -1;
    ...
    void injectKeyUp(const KeyboardEvent& evt)
    {
        Keycode key = evt.keysym.sym;
        // keep track of the player's intended direction
        if (key == 'z' && mKeyDirection.z == -1) mKeyDirection.z = 0;
        else if (key == 'q' && mKeyDirection.x == -1) mKeyDirection.x = 0;
...
private:
    ...
    void setupBody(SceneManager* sceneMgr)
    {
        ...
        mBodyEnt = sceneMgr->createEntity("SinbadBody_", "Sinbad.mesh");
